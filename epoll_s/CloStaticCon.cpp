...
// 定时回调函数,删除非活动连接socket上的注册事件,并关闭
...
int main(int argc, char *argv[]){
        ...
        // 处理信号
            ...
            if(ret == -1){
                ...
                for(int i = 0;i < ret; i++){
                    switch(signals[i]){
                        case SIGALRM:{
                                         // 用timeout变量标记有定时任务需要处理,但不立即处理定时任务,定时任务的优先级不高,需要优先处理其他更重要的任务
                                         timeout = true;
                                         break;
                                     }
                        case SIGTERM:{
    ...
    // 最后处理定时事件,IO事件有更高的优先级。这样做将导致定时任务不能精确地按照预期的时间执行
    if(timeout){
        timer_handler();
        timeout = false;
    }
