...
int main(int argc,char **argv){
    ...
    // 创建users数组,分配FD_LIMIT个client_data对象,每个可能的socket连接都可以获得一个这样的对象,并且socket的值可以用来直接索引
    // socket连接对应的client_data对象,这是将socket和客户数据关联的简单而高效的方式
    client_data* users = new client_data[FD_LIMIT];
    // 尽管分配了足够多的client_data对象,但为了提高poll性能，仍然有必要限制用户的数量
