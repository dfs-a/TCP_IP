...
class time_wheel{
    public:
        ...
        tw_timer* add_timer(int timeout){
            ...
            int ticks = 0;
            // 根据待插入定时器的超时值计算它将在时间轮转动多少个滴答后被触发,并将该滴答数存于变量ticks中
            // 如果待插入定时器的超时值小于时间轮的槽间隔SI,则将ticks向上折合为1,否则就将ticks向下这和为timeout/SI
            if(timeout < SI){
                ticks = 1;
            }else{
                ticks = timeout / SI;
            ...
            // 创建新的定时器,在时间轮转动rotation圈之后被触动,且位于第ts个槽上
